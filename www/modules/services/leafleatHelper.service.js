...
angular.module('services')
  .config(function() {
    L.TileLayer.Offline = L.TileLayer.extend({
      _tileOnError: function(done, tile, e) {
        ...
        var matches = tile.src.match(/[0-9]+/g);
        var coords = {
          y: matches[matches.length - 1],
          x: matches[matches.length - 2],
          z: matches[matches.length - 3]
        };
        var s = this._getSubdomain(coords);

        tile.src = 'http://' + s + '.tile.osm.org/' + coords.z + '/' + coords.x + '/' + coords.y + '.png';
  ...
  .factory('leafletHelper', function($q) {
    ...
    function createMap(id, opts, type) {
      var promise = $q(function(success) {
        ...
        L.offlineTileLayer(preset.map[type].tileLayer, {
